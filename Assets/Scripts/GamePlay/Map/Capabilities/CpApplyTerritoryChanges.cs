...
namespace GamePlay.Map
{
    ...
    public class CpApplyTerritoryChanges : CapabilityBase
    {
        ...
        // 放在规则结算阶段，晚于 CpOccupy 写请求，早于表现阶段的 CpDrawMap 读脏格。
        ...
        {
            context.QuerySnapshotByIds(m_Entities, m_GridId, m_TerritoryOwnershipBufferId,
                m_TerritoryPaintStateId, m_NationIndexId);
        ...
        private void TickOne(CapabilityContext context, CEntity entity)
        {
            ...
            if (!entity.TryGetGrid(out var grid)) return;
            if (!entity.TryGetTerritoryOwnershipBuffer(out var ownershipBuffer)) return;
            if (!entity.TryGetTerritoryPaintState(out var paintState)) return;
            if (!entity.TryGetNationIndex(out var nationIndex)) return;
            ...
            for (int i = 0; i < ownershipBuffer.Changes.Count; i++)
            {
                ...
                // 只有归属真正变化时才标脏，避免 CpDrawMap 做无意义颜色刷新。
        ...
        {
            // HashSet 负责去重，List 负责给 CpDrawMap 提供紧凑遍历。
        ...
        {
            // 全图脏时不需要保存单格脏列表，避免 CpDrawMap 误走增量路径。
