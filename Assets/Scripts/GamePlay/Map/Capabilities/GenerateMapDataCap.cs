...
namespace GamePlay.Map
{
    ...
    public class GenerateMapDataCap : CapabilityBase
    {
        ...
        {
            if (!Owner.HasComponent(m_MapId)) return false;
            if (!Owner.HasComponent(m_NoiseId)) return false;
            if (!Owner.HasComponent(m_BiomeId)) return false;
            return true;
        }
        ...
        {
            if (!Owner.TryGetMap(out var map)) return;
            if (!Owner.TryGetNoise(out var noise)) return;
            if (!Owner.TryGetBiome(out var biome)) return;
