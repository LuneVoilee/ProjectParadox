...
namespace GamePlay.Strategy
{
    ...
    public class CpSelection : CapabilityBase
    {
        private readonly System.Collections.Generic.List<CEntity> m_Events =
        ...
        public override string DebugCategory => CapabilityDebugCategory.Input;
            ...
            (CapabilityContext context, float deltaTime, float realElapsedSeconds)
        {
            ...
            context.QuerySnapshot<GameplayClickEvent>(m_Events);
            for (int i = 0; i < m_Events.Count; i++)
            {
                CEntity eventEntity = m_Events[i];
